...
static void initEngine(const Options &options)
{
    ...
    SDL_WM_SetCaption(branding.getValue("appName", "elektrOGame").c_str(),
                      NULL);
...
#else
    icon = IMG_Load(resman->getPath(branding.getValue("appIcon", "data/icons/elektro.png")).c_str());
    if (icon)
    {
...
std::string LANG = ("LANG=" + std::string(_nl_locale_name_default()));
    putenv(LANG.c_str());
...
int main(int argc, char *argv[])
{
    ...
    // Configure logger
    logger = new Logger;
    logger->setLogFile(homeDir + std::string("/elektro.log"));

    // Log the tmw version
    logger->log("elektrOGame : %s", FULL_VERSION);
    ...
    sound.playMusic(branding.getValue("loginMusic", "Moonlight.ogg"));
