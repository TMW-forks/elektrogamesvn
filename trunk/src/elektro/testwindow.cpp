    ...
    Window(LBLNPC)
{
    ...
    mStartOk = new Button("Hemen Baþla","startok",this);
    mStartCancel = new Button("Yok Almayým","startcancel",this);
...
TestDialog::start()
{
    ...
    switch(testState)
    {
        ...
        case MANYQ_START_SATATE:
            ...
            mStartBox->clearRows();
            mStartBox->addRow("##3==========================================");
            mStartBox->addRow("               ##1Teste Hoþgeldin!");
            mStartBox->addRow("##3==========================================");
            mStartBox->addRow("");
            mStartBox->addRow("  ##2Soru Sayýsý     :##6"+toString(mTotalQuestion));
            mStartBox->addRow("  ##2Toplam Süre     :##6"+toString(mTotalTime)+"##2 saniye");
            mStartBox->addRow("  ##2Baþarý Sýnýrý   :##6"+toString(mSuccessLimit)+"##2 soru");
            mStartBox->addRow("  ##2Soru Baþýna Süre:##6"+toString(mTotalTime/mTotalQuestion)+"##2 saniye");
...
TestDialog::deleteQuestionButtons()
{
    for(miButton = mvButton.begin();
        ...
        ++miButton)
    {
        delete(*miButton); //eski buttonlarý sil
    }
    mvButton.clear();
}
...
//    chatWindow->chatLog(event.getId().substr(0,13), BY_SERVER);
    ...
    if (event.getId() == "ok")
    {
        ...
        if (mTotalQuestion==1)
        {
            if(mType=="radio")
            {
                int answer=1;  //boþ býrakýlmýþsa yanlýþ kabul et
                for (miRadio = mvRadio.begin();
                    miRadio != mvRadio.end();
                    ++miRadio)
                {
...
//                int tru=0;  //boþ býrakýlmýþsa yanlýþ kabul et
//                int sayac=0;
//                for (miCheck = mvCheck.begin();
//                        miCheck != mvCheck.end();
//                        ++miCheck)
...
//// tru + i->size() diyerek her yanlis sayýsýna göre deðerlendirme yapýlabilir
//                tru<0 ? tru=0: tru++;
    ...
    {
        int answer=2;  //boþ býrakýlmýþsa yanlýþ kabul et
        for (miRadio = mvRadio.begin();
            miRadio != mvRadio.end();
            ++miRadio)
        {
...
//!!!!!!!!!!!!!!!!!!
//        current_npc->dialogChoice(i+2); //iptal ve deðerlendir'i geçmek için +2
        Net::getNpcHandler()->nextDialog(current_npc);
        current_npc=0;
        reset();
    }
    else if (ch=="go_prev")
    {
        ...
        {
//!!!!!!!!!!!!!!!!!!!!!!!!
//        current_npc->dialogChoice(mQuestionNumber+1); //iptal ve deðerlendir'i geçmek için +2-1
        Net::getNpcHandler()->nextDialog(current_npc);
        current_npc=0;
        reset();
        }
    }    else if (ch=="go_next")
    {
        ...
        {
//!!!!!!!!!!!!!!!!!!
//        current_npc->dialogChoice(mQuestionNumber+3); //iptal ve deðerlendir'i geçmek için +2+1
        Net::getNpcHandler()->nextDialog(current_npc);
        current_npc=0;
        reset();
        }
...
TestDialog::parse()
{
    ...
    for_each_xml_child_node(node, rootNode)
    {
        ...
        {
            // teste baþlarken yapýlmasý gerekenleri burada yap
            ...
            for_each_xml_child_node(subnode, node)
            {
                ...
                else if (xmlStrEqual(subnode->name, BAD_CAST "testpro"))
                {
                    ...
                    //todo: dereceli odul  ceza için buraya ekleme yapýlacak
...
// iki sütundan birisini seçerek yapýlan karþýlaþtýrma sorularý eklenebilir
// xml'e grup deðeri eklemek yeterli olur
...
//            mScrollArea->setHorizontalScrollPolicy(XML::getProperty(node, "scrollh",2));
        ...
        else if (xmlStrEqual(node->name, BAD_CAST "report"))
        {
            ...
            mFinishText->addRow("    ##1      -= Test BÝTTÝ! =-");
            mFinishText->addRow("##2----------------------------------");
            mFinishText->addRow("Doðru Cevap:"+toString(mDogru));
            mFinishText->addRow("Mesaj      :"+mmMesaj);
            mFinishText->addRow("##2----------------------------------");
            mFinishText->addRow("##3   SN     Doðru C.    Sizin C.   ");
            mFinishText->addRow("##2  ----    --------    --------   ");
            for_each_xml_child_node(subnode, node)
            {
